...
void check_reduct(ArrayBitSet &reduct, AF &framework, uint32_t query_argument, bool &is_rejected, bool &is_terminated, std::__cxx11::list<uint32_t> &extension_build,
	std::__cxx11::list<uint32_t> &certificate_extension, PriorityStackManager &prio_queue, IPrioHeuristic &heuristic, bool isMain)
{
	...
	PriorityStackManager &prio_queue, bool isMain)
{
	...
	if (extension_build.empty()) {
		...
		check_reduct(reduct, framework, query_argument, is_rejected, is_terminated, extension_build,
			certificate_extension, prio_queue, heuristic, isMain);
	}
	else {
		reduct = Reduct::get_reduct_set(active_args, framework, extension_build);
		if (reduct._array.size() < 2)
		{
			// there is only 1 active argument, this has to be the argument to check, 
			// if not then there should have been a rejection check earlier who did not work
		...
		default:
			check_reduct(preprocessed_reduct, framework, query_argument, is_rejected, is_terminated, extension_build,
				certificate_extension, prio_queue, heuristic, isMain);
			break;
		}
	}
