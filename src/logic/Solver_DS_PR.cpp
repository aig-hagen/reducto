...
static bool start_checking_rejection(uint32_t query_argument, AF &framework, ArrayBitSet &active_args_in_coi,
	list<uint32_t> &grounded_extension, ConeOfInfluence &coi, list<uint32_t> &out_certificate_extension)
{
...
bool Solver_DS_PR::solve(uint32_t query_argument, AF &framework, list<uint32_t> &out_certificate_extension)
{
	...
	default:
		return !start_checking_rejection(query_argument, framework, active_args_in_coi, grounded_extension, coi, out_certificate_extension);
	}
}