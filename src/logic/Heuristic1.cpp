...
uint32_t Heuristic1::calculate_priority(AF &framework, list<uint32_t> &extension, list<uint32_t> &initialSet, uint32_t query, ConeOfInfluence &coi) {

	if (extension.empty()) {
		cout << "calculate_priority:: extension must not be empty" << endl;
		throw new invalid_argument("extension must not be empty");
	}
	uint32_t prio = coi.Distance_to_query[*extension.begin()];
